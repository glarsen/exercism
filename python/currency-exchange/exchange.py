...
def exchangeable_value(budget: float, exchange_rate: float, spread: int, denomination: int) -> int:
    ...
    real_rate = exchange_rate + fee

    absolute_exchangeable = exchange_money(budget, real_rate)
    not_exchangeable = get_leftover_of_bills(absolute_exchangeable, denomination)

    return int(absolute_exchangeable - not_exchangeable)
